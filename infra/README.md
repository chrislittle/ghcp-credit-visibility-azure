...
- Azure SQL supports **either serverless or provisioned** — choose via `sql_database_sku` (`deploy.ps1` prompts for this explicitly). Default is **serverless (`GP_S_Gen5_1`)** with **auto-pause on by default** (`sql_auto_pause_minutes = 60`), which is billed only for storage while paused. Serverless is priced ~3.4x the per-vCore-hour rate of an equivalent provisioned SKU (eastus2), so it's only cheaper if the DB is idle a meaningful fraction of the time — if it runs 24x7, switch `sql_database_sku` to a **provisioned** SKU (e.g. `GP_Gen5_2`) instead, which is cheaper and predictable for always-on workloads. Set `sql_auto_pause_minutes = -1` only if you need always-warm serverless (avoids first-hit resume latency for the SQL Entra admin grant/migrations/snapshot job) — but at that point provisioned is almost always the better/cheaper choice. Swap to Table Storage if pure key/value persistence is preferred (cheaper, but the trend/query code assumes SQL).
